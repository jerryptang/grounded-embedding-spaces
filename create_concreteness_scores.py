...
def similarity_scores(w, sigma, vocab, vocab_ids):
    '''Returns the similarity of each word in vocab to w, sorted in descending order.'''
    vec = sigma[vocab_ids[w]]
    return sorted(zip(vocab, vec), key = lambda x : -x[1])

def interpolate_conc_scores(vocab, vocab_ids, sigma_A, conc_ratings, top_nn = 15):
    '''Compute concreteness scores for each word in vocab, using amodal similarities to interpolate scores for missing words.'''
    scores = {}
    for w in vocab:
        ...
        nn_correlations = similarity_scores(w, sigma_A, vocab, vocab_ids)
...
if __name__ == '__main__':
    ...
    parser.add_argument('semantic_priors_file', type=str)
    parser.add_argument('outfile', type=str)
    args = parser.parse_args()

    # load story words and amodal embedding space
    data = np.load(args.semantic_priors_file)
    vocab = data['vocab']
    ...
    conc_scores = interpolate_conc_scores(vocab, vocab_ids, sigma_A, conc_ratings_dict, top_nn = 15)
